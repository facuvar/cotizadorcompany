<?php
/**
 * Script DIRECTO para importar datos a Railway
 * Usa estrategia de expresiones regulares específicas
 */

echo "<h1>🎯 IMPORTACIÓN DIRECTA - Railway MySQL</h1>";
echo "<style>
...
// Leer archivo SQL
echo "<div class='step'>";
echo "<h2>📄 Procesando Archivo SQL con Regex</h2>";
...
// Extraer CREATE TABLE usando regex específica
preg_match_all('/CREATE TABLE[^;]+;/is', $sqlContent, $createMatches);
$createQueries = $createMatches[0];

// Extraer INSERT INTO usando regex específica
preg_match_all('/INSERT INTO[^;]+;/is', $sqlContent, $insertMatches);
$insertQueries = $insertMatches[0];

// Extraer DROP TABLE usando regex específica
preg_match_all('/DROP TABLE[^;]+;/is', $sqlContent, $dropMatches);
$dropQueries = $dropMatches[0];
...
if (!empty($createQueries)) {
    ...
    $example = substr($createQueries[0], 0, 200) . '...';
...
if (!empty($insertQueries)) {
    ...
    echo "<div class='debug'>";
    $example = substr($insertQueries[0], 0, 200) . '...';
    echo htmlspecialchars($example);
    echo "</div>";
}
echo "</div>";
...
if ($totalRecords > 0) {
    ...
    echo "<p>No se importaron datos. Puede ser necesario revisar el formato del archivo SQL.</p>";
    echo "</div>";
}
