 ...
 * Convierte UTF-16 LE a UTF-8 y ordena consultas según dependencias
 */

echo "<h1>🎯 IMPORTACIÓN ORDENADA - UTF-16 a UTF-8</h1>";
echo "<style>
...
// Ordenar CREATE TABLE según dependencias
echo "<div class='info'>🔄 Ordenando CREATE TABLE según dependencias...</div>";

// Definir orden de dependencias conocido
$tableOrder = [
    'categorias',
    'plazos_entrega', 
    'productos',
...
if ($totalQueries > 0) {
    ...
    try {
        ...
        foreach ($allQueries as $index => $query) {
            ...
            // Actualizar progreso cada 5 consultas
            if ($index % 5 == 0) {
                echo "<script>
...
$tables = ['categorias', 'opciones', 'plazos_entrega', 'presupuestos', 'opcion_precios', 'productos', 'adicionales', 'xls_productos', 'xls_opciones', 'xls_precios'];
...
if ($totalRecords > 0) {
    ...
    echo "<p>Las tablas se crearon pero no se importaron datos. Esto puede ser normal si el archivo solo contiene estructura.</p>";
    echo "</div>";
}
