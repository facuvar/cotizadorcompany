...
function getSheetData($sheetId, $gid = 0) {
    ...
    if ($data === false) {
        error_log("Error al obtener datos de la hoja con GID $gid: " . error_get_last()['message']);
        return false;
    }
    ...
    if (count($rows) <= 1) {
        error_log("La hoja con GID $gid está vacía o no tiene suficientes datos");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sheet_url'])) {
    ...
    if ($sheetId) {
        ...
        // Intentar obtener datos de la primera hoja para verificar que el archivo sea accesible
        $testData = getSheetData($sheetId, 0);
        if (!$testData || count($testData) <= 1) {
            $mensaje .= mostrarMensaje("No se pudo acceder al archivo de Google Sheets o está vacío. Asegúrese de que el archivo sea público y tenga el formato correcto.", "error");
            return;
        }
        
        $mensaje .= mostrarMensaje("Archivo de Google Sheets accesible. Iniciando importación...", "success");
        ...
        try {
            ...
            foreach ($sheets as $sheetName => $gid) {
                ...
                foreach ($data as $index => $row) {
                    ...
                    // Si la primera columna tiene contenido y la segunda está vacía, probablemente es un producto
                    // O si contiene palabras clave específicas
                    if (!empty($firstCell) && (empty($row[1]) || 
                        ...
                        strpos(strtoupper($firstCell), 'OPCION') !== false ||
                        strpos(strtoupper($firstCell), 'PEFIL') !== false ||
                        strpos(strtoupper($firstCell), 'CABINA') !== false ||
                        strpos(strtoupper($firstCell), 'RESTAR') !== false ||
                        strpos(strtoupper($firstCell), 'INDICADOR') !== false ||
                        strpos(strtoupper($firstCell), 'BALANZA') !== false ||
                        strpos(strtoupper($firstCell), 'INTERCOMUNICADOR') !== false ||
                        strpos(strtoupper($firstCell), 'FASE') !== false ||
                        strpos(strtoupper($firstCell), 'EXTENSION') !== false ||
                        strpos(strtoupper($firstCell), 'PARADA') !== false ||
                        strpos(strtoupper($firstCell), 'MISMA') !== false ||
                        strpos(strtoupper($firstCell), 'DOMICILIARIO') !== false ||
                        strpos(strtoupper($firstCell), 'MONTAVEHICULO') !== false
                    )) {
